...
def finalize_load(actor: Any, checkpoint_payload: dict[str, Any] | None) -> None:
    ...
    if checkpoint_payload.get("rng") is not None and not getattr(actor.args, "no_load_rng", False):
        ...
        if rng_key in rng_state and accelerator.accelerator_module() is not None:
            accelerator.set_rng_state_all(rng_state[rng_key])
...
def save(actor: Any, iteration: int) -> None:
    ...
    if dist.get_rank() == 0:
        ...
        if accelerator.accelerator_module() is not None:
            rng_state[accelerator.device_type()] = accelerator.get_rng_state_all()
